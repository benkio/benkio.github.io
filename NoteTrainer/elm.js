...
var $author$project$Main$bpmToMillis = function (bpm) {
	return 60000 / bpm;
};
...
var $author$project$Main$subscriptions = function (model) {
	var _v0 = model.isPlaying;
	if (_v0) {
		return A2(
			$elm$time$Time$every,
			$author$project$Main$bpmToMillis(model.bpm),
			function (_v1) {
				return $author$project$Main$ChangeNote;
			});
	} else {
...
var $author$project$Filter$byNoteTonalityGenerator = function (notes) {
	return A2(
	...
	if (filter.$ === 'ChromaticScale') {
		var chordGenerator = A2(
			...
			A2(
				...
				$elm$core$Maybe$withDefault($author$project$Music$majorChord)),
...
var $author$project$Wave$waveToString = function (w) {
	switch (w.$) {
		case 'Sine':
			return 'sine';
		case 'Triangle':
		...
		_List_fromArray(
			[
				...
				$elm$json$Json$Encode$string(
					$author$project$Wave$waveToString(oscillatorWave)))
			]));
};
var $author$project$Wave$Sawtooth = {$: 'Sawtooth'};
var $author$project$Wave$Square = {$: 'Square'};
var $author$project$Wave$Triangle = {$: 'Triangle'};
var $author$project$Wave$toWave = function (s) {
	switch (s) {
		case 'sine':
			return $elm$core$Maybe$Just($author$project$Wave$Sine);
		case 'triangle':
			return $elm$core$Maybe$Just($author$project$Wave$Triangle);
		case 'square':
			return $elm$core$Maybe$Just($author$project$Wave$Square);
		case 'sawtooth':
			return $elm$core$Maybe$Just($author$project$Wave$Sawtooth);
		default:
			return $elm$core$Maybe$Nothing;
	}
};
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							oscillatorWave: A2(
								$elm$core$Maybe$withDefault,
								$author$project$Wave$Sine,
								$author$project$Wave$toWave(wave))
						}),
					$elm$core$Platform$Cmd$none);
			case 'FilterChange':
				var filter = msg.a;
			...
			default:
				...
				return _Utils_Tuple2(
					...
					$author$project$Main$play(
						$author$project$Main$toMusic(newModel)));
		}
	});
...
var $author$project$Main$startButton = function (isPlaying) {
	var _v0 = function () {
		if (isPlaying) {
			return _Utils_Tuple3($author$project$Main$Stop, 'btn-danger', 'Stop');
		} else {
			return _Utils_Tuple3($author$project$Main$Start, 'btn-success', 'Start');
		}
	}();
	var msg = _v0.a;
	var btnClass = _v0.b;
					...
					_List_fromArray(
						[
							...
							A2(
								$elm$core$Basics$composeR,
								$elm$core$Maybe$map(
									$elm$core$String$append('Tonality ')),
								$elm$core$Maybe$withDefault('By Tonality'))(tonalityKey)),
							A2(
							$elm$html$Html$span,
							_List_fromArray(
								[
...
var $author$project$Main$panelBody = function (model) {
	var _v0 = function () {
		...
		if (_v1.$ === 'ChromaticScale') {
			return _Utils_Tuple3('btn-primary', 'btn-link', $elm$core$Maybe$Nothing);
		} else {
			var n = _v1.a;
			return _Utils_Tuple3(
				'btn-link',
	...
	var tonalityClass = _v0.b;
	var tonalityKey = _v0.c;
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
