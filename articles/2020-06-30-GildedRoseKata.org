...
*** Solution Approach
    ...
    Following up the previous analysis, we can come up with a plan and
    in here I'll prioritize the tasks we need to do in order to make
...
**** Tests
     ...
     The first task needed is a good test suite. With a proper testing we
     ...
     Now, test suite can be crafted using multiple techniques and it's an
     ...
     operation. Definitely, the most complicated part is identify the
...
**** Split the Monster
     ...
     Once we have the testing setup properly, we can start to break
     stuff using the 🔨.

     The Most annoying thing into the codebase is the giant
     stand-alone function. I will tackle this by splitting that
     function in two or more sub-functions. Then, repeat the same
     approach for the out coming functions. This recursive process
     will terminate when the functions I end up with have a small
     amount of lines of code and one single mutation in it, simple as
     that.
...
// After
def f1(params): StuffType = if (condition1) { f3(params) } else // Previous state (indentity)
...
#+END_SRC
     ...
     into put the computations in context. This way, you don't need to
     ...
     function will behave as the [[https://en.wikipedia.org/wiki/Identity_function][Indentity Function]].

     In the very end, I want also to grab your attention on the way
     this approach penetrates the nested ifs and allow to collapse the
...
**** Dealing With Mutation once and for All
     ...
     Everything is in it's own box that returns a new version of the
     input 💜 However... it still mutate and internal field!! But
...
**** Refinement
     ...
     + Structuring the code: moving function to the model companion
