...
*** Parallel Collection
  ...
  #+begin_src scala
    ...
    inputFiles.foreach {
      case (inputFile, filename) => {
        val ffmpegExitCode = ffmpegCommand(inputFile, filename).exitCode
        if (ffmpegExitCode == 0) {
          val splitExitCode = splitCommand(filename).exitCode
          if (splitExitCode == 0) moveCommand(filename)
...
*** Testing
  ...
  #+bengin_src scala
  def tempPath(inputFile:String) = os.home/"temp"/"test"/inputFile
  def startingPath(inputFile:String): os.Path = os.home/"temp"/"test"/inputFile
  val destinationPath = os.home/"temp"/"convertedFilms"
  #+end_src_
