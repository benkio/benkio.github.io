...
#+begin_src javascript :exports none
  // Pagination 0 based
  const siteUrl = "https://benkio.github.io"
  ...
  for (var i = 0; i < htmlArticlesPathPaginated.length; i++) {
      var pageUrl = siteUrl + htmlArticlesPathPaginated[i].slice(1)
    ...
    .then(html => {
      ...
      return [
          ...
          doc.querySelector('"#text-ArticleAbstract p"').innerHTML
    ...
    articlesZip.push(promise);
  }

  await Promise.all(articlesZip).then(function (tuple) {
